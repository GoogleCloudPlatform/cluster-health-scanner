...
WORKDIR /scripts
RUN apt-get update && apt-get install -y sudo openssh-server python3.9 ca-certificates curl python3-pip &&\
...
COPY src/nccl_pairwise/config.proto /scripts/
...
COPY src/nccl_pairwise/nccl_startup.py /scripts/
COPY src/nccl_pairwise/config.py /scripts/
COPY src/checker_common.py /scripts/
COPY src/nccl_pairwise/a3/ a3/
COPY src/nccl_pairwise/a3plus/ a3plus/
COPY src/nccl_pairwise/run-nccl-combined-plugins.sh .
