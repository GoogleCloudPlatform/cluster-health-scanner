...
run_nccl_fastrak() {
  ...
  #LD_LIBRARY_PATH=${ld_library_path_override} \
  LD_LIBRARY_PATH=/usr/local/lib:/var/lib/tcpxo/lib64:/usr/lib/x86_64-linux-gnu:$LD_LIBRARY_PATH \
  mpirun --mca pmi pmix --mca btl tcp,self --mca btl_tcp_if_include enp0s12 --allow-run-as-root \
    -np $(( gpu_per_node * "${nhosts}" )) \
