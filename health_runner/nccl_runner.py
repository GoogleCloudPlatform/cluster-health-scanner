...
_NCCL_INTER_RACK_YAML_PATH = os.environ.get(
    "NCCL_TOPOLOGY_YAML_PATH", "/app/nccl_topology.yaml"
)
...
def run_nccl_random_pair_healthcheck(num_tests: int, second_pass_enabled: bool):
  ...
  checker_common.run_command(
      "/app/kubectl label nodes -l aiinfra/nccl-healthcheck-valid-till-sec"
      " aiinfra/nccl-healthcheck-valid-till-sec-"
  )

  second_pass_yaml_path = os.path.join(
      "/app", second_pass_file
  )
...
def run_intra_rack_healthcheck(
  ...
  for rack, nodes in rack_to_nodes.items():
    ...
    for i, j in node_pairs:
      node1 = nodes[i]
      node2 = nodes[j]
      job_name = f"nccl-healthcheck-{uuid.uuid4()}-{node1}-{node2}"
  ...
  passed_nodes, failed_nodes = get_nccl_test_results(v1)
  ...
  for rack, nodes in rack_to_nodes.items():
    ...
    for failed_node in failed_nodes_in_rack:
      ...
      job_name = f"nccl-healthcheck-{uuid.uuid4()}-{healthy_node}-{failed_node}"
      checker_common.create_k8s_objects(
          _NCCL_INTER_RACK_YAML_PATH,
          _KUBECTL,
          mappings={
  ...
  second_passed_nodes, second_failed_nodes = get_nccl_test_results(v1)
...
def run_inter_rack_healthcheck(
  ...
  passed_nodes, _ = get_nccl_test_results(v1)
...
def get_nccl_test_results(
  ...
  for node in nodes.items:
    if node.metadata.labels.get("aiinfra/nccl-healthcheck-result") == "pass":
      passed_nodes.add(node.metadata.name)
    elif node.metadata.labels.get("aiinfra/nccl-healthcheck-result") == "fail":
      failed_nodes.add(node.metadata.name)
    elif node.metadata.labels.get("aiinfra/nccl-healthcheck-result") == "crash":
      failed_nodes.add(node.metadata.name)
    else:
...
# Example usage:
list_length = 5
result = generate_index_pairs(list_length)
print(result)
...
def get_nodes_data(
  ...
  for kube_node in kube_nodes:
    if "topology.gke.io/cluster" not in kube_node.metadata.labels:
      print(f"Skipping node {kube_node.metadata.name} due to missing label.")
      continue
